...
export default function MyHolidaysPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Only admin and manager can use this page
  ...
  const handleCancelRequest = async (req: any) => {
    if (req.status === 'pending') {
      ...
      const confirmed = confirm('Send a cancellation request? Your days will be returned if approved.')
      if (!confirmed) return
    ...
    <main className="min-h-screen bg-gray-100">
      ...
      <div className="bg-blue-700 text-white px-6 py-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-blue-200 text-sm">My Holidays</p>
        </div>
        <button
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-4xl font-bold mt-1">
                {balance} <span className="text-lg font-normal">days</span>
              </p>
              <p className="text-xs opacity-80 mt-1">
                Working {workingDaysCount} days per week
              </p>
            </div>
              ...
              <ul className="space-y-2">
                {requests.map((req) => (
                  <li
                    ...
                    className="bg-white rounded-2xl shadow p-4 border border-gray-100 cursor-pointer hover:bg-gray-50 active:bg-gray-100 transition"
                  >
                    <div className="flex items-center justify-between">
