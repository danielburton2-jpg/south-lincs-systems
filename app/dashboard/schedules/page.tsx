...
export default function SchedulesPage() {
  ...
  if (loading) {
    return (
      <div className="p-8 text-slate-400 italic">Loading schedules…</div>
    )
  }
  ...
  return (
    <div className="p-8 max-w-6xl">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Schedules</h1>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-center flex-wrap gap-3">
          <div>
            <h2 className="text-base font-semibold text-slate-700">Active Schedules</h2>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 flex flex-wrap gap-3 items-center">
          <input
            ...
            className="flex-1 min-w-[200px] border border-slate-300 rounded-xl px-3 py-2 text-sm text-slate-900"
          />
                ...
                <tbody>
                  {visibleSchedules.map(s => (
                    <tr
                      ...
                      className={`border-b border-slate-100 last:border-b-0 cursor-pointer hover:bg-slate-50 transition ${!s.active ? 'opacity-60' : ''}`}
                    >
                      ...
                      <td className="px-4 py-3">
                        {s.active ? (
                          ...
                          <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-medium">Inactive</span>
                        )}
                      </td>
                    </tr>
                  ))}
            ...
            <div className="md:hidden divide-y divide-slate-100">
              {visibleSchedules.map(s => (
                <div
                  key={s.id}
                  onClick={() => router.push(`/dashboard/schedules/${s.id}`)}
                  className={`p-4 cursor-pointer hover:bg-slate-50 transition ${!s.active ? 'opacity-60' : ''}`}
                >
                  ...
                  {s.schedule_type === 'recurring' && (
                    <div className="flex gap-1 flex-wrap mt-2 ml-7">
                      {getRecurringDayPills(s.recurring_days).map(d => (
                        <span key={d} className="text-[10px] bg-slate-100 text-slate-700 px-1.5 py-0.5 rounded">
                          {d}
                        </span>
                      ))}
                    </div>
