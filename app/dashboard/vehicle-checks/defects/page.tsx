...
export default function AdminDefectsPage() {
  ...
  if (loading) {
    return (
      <div className="p-8 text-slate-400 italic">Loading defects…</div>
    )
  }
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="mb-6 flex items-baseline justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 mb-1">Vehicle Defects</h1>
      ...
      <div className="space-y-4">
            ...
            return (
              <button
                ...
                className={`rounded-xl border-2 p-3 text-center transition ${
                  statusFilter === s ? colors[s] : 'bg-white border-slate-200 hover:border-slate-300'
                }`}
              >
                <p className={`text-2xl font-bold ${statusFilter === s ? textColors[s] : 'text-slate-700'}`}>{counts[s]}</p>
                <p className={`text-xs mt-0.5 capitalize font-medium ${statusFilter === s ? textColors[s] : 'text-slate-500'}`}>
                  {s}
                </p>
              </button>
            )
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-2">
          <input
            ...
            className="w-full border-0 px-3 py-2 text-sm text-slate-900 focus:outline-none"
          />
        </div>
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-xl shadow border ${
                    d.status === 'open' ? 'border-red-200' : 'border-slate-200'
                  }`}
                >
                  <div className="p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {v?.name && <p className="text-xs text-slate-500 mt-0.5">{v.name}</p>}

                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mt-3">{d.category}</p>
                        ...
                        {d.defect_note && (
                          <div className="mt-1.5 bg-slate-50 border border-slate-200 rounded-lg p-2">
                            <p className="text-xs text-slate-700 leading-snug whitespace-pre-wrap">{d.defect_note}</p>
                          </div>
                        )}
                  ...
                  {isResolving && (
                    <div className="border-t border-slate-200 p-4 bg-slate-50 space-y-3">
                      ...
                      <div className="flex gap-2 justify-end">
                        <button onClick={cancelResolve} disabled={submitting} className="text-sm bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg disabled:opacity-50">Cancel</button>
                        <button
                  ...
                  {isAssigning && (
                    <div className="border-t border-slate-200 p-4 bg-purple-50 space-y-3">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <p className="text-sm font-semibold text-slate-800">🔧 Assign to a Mechanic</p>
                        <p className="text-xs text-slate-500">Showing users with "Mechanic" in job title</p>
                      </div>
                      ...
                      {mechanics.length === 0 ? (
                        ...
                        <>
                          ...
                          />
                          <div className="max-h-60 overflow-y-auto bg-white rounded-lg border border-slate-200 divide-y divide-slate-100">
                            {mechanicsForAssign.length === 0 ? (
                              <p className="p-3 text-xs text-slate-500 text-center">No mechanics match</p>
                              ...
                              return (
                                <button
                                  ...
                                  className={`w-full text-left p-2 hover:bg-slate-50 disabled:opacity-50 ${isCurrent ? 'bg-purple-100' : ''}`}
                                >
                                  <div className="flex items-center justify-between gap-2 flex-wrap">
                                  ...
                                  {u.job_title && <p className="text-xs text-slate-500">{u.job_title}</p>}
                                </button>
                              )
                            })}
                          </div>
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        <button onClick={cancelAssign} disabled={submitting} className="text-sm bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg disabled:opacity-50">Close</button>
                      </div>
                    </div>
                  )}

                  {isAddingNote && (
                    <div className="border-t border-slate-200 p-4 bg-amber-50 space-y-3">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button onClick={cancelAddNote} disabled={submitting} className="text-sm bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg disabled:opacity-50">Cancel</button>
                        <button
                  ...
                  {!isResolving && !isAssigning && !isAddingNote && (
                    <div className="border-t border-slate-200 p-3 flex justify-end gap-2 flex-wrap">
                      {d.status === 'open' ? (
                        <>
                          <button
