        ...
        {activeTab === 'checks' ? (
          filteredChecks.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-sm p-8 border border-slate-100 text-center">
              ...
              <p className="text-slate-700 font-medium">No submitted checks yet</p>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 divide-y divide-slate-100 overflow-hidden">
              {filteredChecks.map(c => (
                <button
                  ...
                  className="w-full text-left p-4 flex items-center gap-3 hover:bg-slate-50 active:bg-slate-100 transition"
                >
          ...
          filteredDefects.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-sm p-8 border border-slate-100 text-center">
              <p className="text-5xl mb-3">✅</p>
              <p className="text-slate-700 font-medium">No defects found</p>
              <p className="text-xs text-slate-500 mt-1">
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-2xl shadow-sm border p-3 ${
                      d.status === 'open' ? 'border-red-200' : 'border-slate-100'
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="font-mono font-bold text-slate-800 text-sm">{v?.registration}</p>
                          {v?.fleet_number && (
                            <span className="text-xs bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full font-medium">
                              #{v.fleet_number}
                            </span>
                          )}
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium border capitalize ${statusColors[d.status] || 'bg-slate-100 text-slate-600'}`}>
                            {d.status}
                          </span>
                        </div>
                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mt-2">{d.category}</p>
