    ...
    <main className="min-h-screen bg-gray-50 pb-32">
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="bg-white hover:bg-gray-50 active:bg-gray-100 rounded-2xl shadow-sm border border-gray-100 p-5 text-center transition"
          >
            <div className="text-4xl mb-2">🆕</div>
            <p className="font-bold text-gray-800">New Check</p>
            <p className="text-xs text-gray-500 mt-0.5">Pick a vehicle</p>
          </button>
          <button
            ...
            className={`rounded-2xl shadow-sm border p-5 text-center transition relative ${
              openDefectsCount > 0
                ...
                : 'bg-white hover:bg-gray-50 active:bg-gray-100 border-gray-100'
            }`}
          >
            <div className="text-4xl mb-2">⚠️</div>
            <p className={`font-bold ${openDefectsCount > 0 ? 'text-red-800' : 'text-gray-800'}`}>Defects</p>
            <p className={`text-xs mt-0.5 ${openDefectsCount > 0 ? 'text-red-600' : 'text-gray-500'}`}>
        ...
        <div>
          ...
          {pendingChecks.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
              ...
              <p className="text-xs text-gray-500 mt-1">No checks in progress.</p>
            </div>
          ) : (
                ...
                <div key={c.id} className="bg-amber-50 border border-amber-200 rounded-2xl shadow-sm overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {c.vehicle?.name && (
                        <p className="text-sm text-gray-600 mt-0.5">{c.vehicle.name}</p>
                      )}
      ...
      <div className="fixed bottom-16 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 shadow-lg">
        <button
          ...
          className="w-full bg-gray-100 hover:bg-gray-200 text-gray-800 py-3 rounded-xl font-medium text-sm"
        >
